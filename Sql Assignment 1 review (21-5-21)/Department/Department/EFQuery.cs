...
namespace Department.UI
{
    public class EFQuery
    {
        ...
        public static void ProductNameCategoryStockPrice()
        {
            ...
            {
                var query = from product in context.Set<Product>()
                            join prodcategory in context.Set<ProductCategory>() on product.ProductId equals prodcategory.ProductId
                            join category in context.Set<Category>() on prodcategory.CategoryId equals category.CategoryId
        ...
        public static void ProductWithinCategory()
        {
            using (DepartmentContext context = new DepartmentContext())
            {
                var query = from category in context.Set<Category>()
                            join prodcategory in context.Set<ProductCategory>() on category.CategoryId equals prodcategory.CategoryId
                            join product in context.Set<Product>() on prodcategory.ProductId equals product.ProductId
        ...
        public static void SupplierDetails()
        {
            using (DepartmentContext context = new DepartmentContext())
            {
                var query = from supplier in context.Set<Supplier>()
                            join supplireprodorder in context.Set<SupplierProductOrderDetail>() on supplier.SupplierId equals supplireprodorder.SupplierId
                            join prodt in context.Set<Product>() on supplireprodorder.ProductId equals prodt.ProductId
                            join address in context.Set<Address>() on supplier.AddressId equals address.AddressId
                            join prodorder in context.Set<ProductOrderDetail>() on supplireprodorder.ProductOrderId equals prodorder.ProductOrderId
        ...
        public static void ProductAndSupplierWithRecentDatesOfSupply()
        {
            ...
            {
                var query = from product in context.Set<Product>()
                            join posupplier in context.Set<SupplierProductOrderDetail>() on product.ProductId equals posupplier.ProductId
                            join supplier in context.Set<Supplier>() on posupplier.SupplierId equals supplier.SupplierId
                            join po in context.Set<ProductOrderDetail>() on posupplier.ProductOrderId equals po.ProductOrderId
                            join inventory in context.Set<Inventory>() on product.ProductId equals inventory.ProductId
